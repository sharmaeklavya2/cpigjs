...
export async function setup(sfUrl, inputUrls, config) {
    ...
    if (procInput.insaneCExs.length > 0) {
        ...
        setupErrorsHtml.appendChild(createElement('span', undefined, 'Counterexamples that contradict implications:'));
...
function addPredParams(output, preds) {
    for (const pred of preds) {
        output.push(new f2f.Param(pred.name, new f2f.CheckBoxWidget({ defVal: false }), { label: pred.label || pred.name }));
    }
}
