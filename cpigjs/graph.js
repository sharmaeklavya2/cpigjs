...
export class Graph {
    ...
    getOutTree(root) {
        ...
        pred = new Map();
        const queue = new CircularQueue(this.adj.size);
        queue.push(root);
        while (queue.size > 0) {
            const u = queue.pop();
    ...
    trCompression(S, toposort = true) {
        ...
        if (toposort) {
            ...
            vertices.sort((u, v) => endTime.get(v) - endTime.get(u));
