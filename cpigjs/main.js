...
export function processInput(input, sf) {
...
export function filterInput(input, sf, rawConstraint) {
    ...
    const trGoodPreds = new MultiMap();
...
export function outputGoodBadReasons(input, predNames, stdout) {
    const mainList = [[input.goodnessName, input.trGoodPreds], [input.badnessName, input.trBadPreds]];
...
export function serializeGraph(input, predNames, showMaybeEdges, format) {
    if (format === 'txt') {
...
export function getDotGraph(input, predNames, showMaybeEdges) {
    const { scc, dag } = input.impG.trCompression(predNames.length > 0 ? predNames : undefined);
    const redDag = dag.trRed();
    const lines = ['digraph G {', 'edge [arrowhead=vee];'];
    for (const u of redDag.adj.keys()) {
        const uAttrs = { 'label': componentStr(scc.get(u), false) };
